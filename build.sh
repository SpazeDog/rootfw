...
LIB_PROJECT="$(dirname "$(readlink -f "$0")")"
BUILD_SRC="$(dirname "$LIB_PROJECT")"
BUILD_NAME=$(basename "$BUILD_SRC")
LIB_DST=~/.gradle/build/$BUILD_NAME/rootfw
LIB_SRC="$LIB_PROJECT/src/main/java"
LIB_FILE="$LIB_DST/outputs/aar/rootfw-$BUILD_TYPE.aar"
LIB_PKG="$LIB_PROJECT/releases/rootfw-$BUILD_TYPE.aar"
DOC_DST="$LIB_PROJECT/docs/dokka"

BINTRAY_DST=~/.gradle/build/$BUILD_NAME/bintray
BINTRAY_PACKAGE="rootfw"
    ...
    if echo $answer | grep -iq "^y"; then
        if which curl > /dev/null 2>&1; then
            ...
            if [ -f "$KEYFILE" ]; then
                ...
                USERNAME="`cat "$KEYFILE" | cut -d : -f 1`"
...
Publishing with user information:
...
if [ -f dokka.jar ]; then
    echo -n "Do you want to generated documentation? [Y/N]: "
    ...
    if echo $answer | grep -iq "^y"; then
        java -jar dokka.jar "$LIB_SRC" -output "$DOC_DST" -format gfm
    fi
fi
